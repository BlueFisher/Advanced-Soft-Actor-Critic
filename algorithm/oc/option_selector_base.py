...
class OptionSelectorBase(SAC_Base):
    ...
    def _build_model(self, nn, nn_config: dict | None, init_log_alpha: float, learning_rate: float) -> None:
        ...
        self._np_padding_action = np.concatenate(d_action_list + [np.zeros(self.c_action_size, dtype=np.float32)], axis=-1)
        self._padding_action = torch.from_numpy(self._np_padding_action).to(self.device)
            ...
            if self.seq_encoder in (None, SEQ_ENCODER.RNN):
                ...
                class ModelRep(nn.ModelOptionSelectorRep):
                    def forward(self, seq_q_len, index, obs_list, *args, **kwargs):
                        if p_self.use_dilation:
                            obs_list = obs_list[0:1] + process_normalizer(obs_list[1:])
                        else:
                            obs_list = process_normalizer(obs_list)
        ...
        for model_target_v in self.model_target_v_over_options_list:
            for param in model_target_v.parameters():
                param.requires_grad = False
    ...
    def _init_or_restore(self, last_ckpt):
        ...
        self.low_state_size = self.option_list[0].state_size
        ...
        for i, option in enumerate(self.option_list):
            ...
            (o_action,
             ...
             o_termination) = option.choose_action(o_low_obs_list,
                                                   ...
                                                   offline_action=offline_action[mask] if offline_action is not None else None,
    ...
    def get_l_states(
        ...
        if self.seq_encoder == SEQ_ENCODER.ATTN and self.use_dilation:
    ...
    def get_obnx_low_states(self,
                            obnx_indexes: torch.Tensor,
                            obnx_padding_masks: torch.Tensor,
                            obnx_low_obses_list: list[torch.Tensor],
                            obnx_option_indexes: torch.Tensor,
                            obnx_pre_actions: torch.Tensor,
                            obnx_pre_low_seq_hidden_states: torch.Tensor = None,
                            is_target: bool = False) -> tuple[torch.Tensor,
                                                              torch.Tensor]:
        """
        Args:
            obnx_indexes (torch.int32): [batch, ob + n + 1]
            obnx_padding_masks (torch.bool): [batch, ob + n + 1]
            obnx_low_obses_list: list([batch, ob + n + 1, *low_obs_shapes_i], ...)
            obnx_option_indexes (torch.int64): [batch, ob + n + 1]
            obnx_pre_actions: [batch, ob + n + 1, action_size]
            obnx_pre_low_seq_hidden_states: [batch, ob + n + 1, *low_seq_hidden_state_shape]

        Returns:
            obnx_low_states: [batch, ob + n + 1, low_state_size]
            obnx_low_seq_hidden_states: [batch, ob + n + 1, *low_seq_hidden_state_shape]
        """
        obnx_low_states = torch.zeros((obnx_indexes.shape[0], obnx_indexes.shape[1], self.low_state_size), device=self.device)
        obnx_low_seq_hidden_states = torch.zeros((obnx_indexes.shape[0], obnx_indexes.shape[1], *self.low_seq_hidden_state_shape), device=self.device)

        option_index = obnx_option_indexes[:, self.option_burn_in_step]
        ...
        for i, option in enumerate(self.option_list):
            ...
            o_obnx_states, o_obnx_low_seq_hidden_states = option.get_l_states(
                l_indexes=obnx_indexes[mask],
                l_padding_masks=torch.logical_or(obnx_padding_masks[mask],
                                                 obnx_option_indexes[mask] != i),
                l_obses_list=[o[mask] for o in obnx_low_obses_list],
                l_pre_actions=obnx_pre_actions[mask],
                l_pre_seq_hidden_states=obnx_pre_low_seq_hidden_states[mask],

                is_target=is_target
            )

            obnx_low_states[mask] = o_obnx_states
            obnx_low_seq_hidden_states[mask] = o_obnx_low_seq_hidden_states

        return obnx_low_states, obnx_low_seq_hidden_states

    @torch.no_grad()
    def get_obn_probs(self,
                      obn_low_obses_list: list[torch.Tensor],
                      obn_low_states: torch.Tensor,
                      obn_option_indexes: torch.Tensor,
                      obn_actions: torch.Tensor) -> torch.Tensor:
        """
        Args:
            obn_low_obses_list: list([batch, l, *low_obs_shapes_i], ...)
            obn_low_states: [batch, l, low_state_size]
            obn_option_indexes (torch.int64): [batch, l]
            obn_actions: [batch, l, action_size]

        Returns:
            obn_low_probs: [batch, l, action_size]
        """
        obn_low_probs = torch.zeros_like(obn_actions)

        option_index = obn_option_indexes[:, self.option_burn_in_step]
        ...
        for i, option in enumerate(self.option_list):
            ...
            o_obn_probs = option.get_l_probs(l_obses_list=[o[mask] for o in obn_low_obses_list],
                                             l_states=obn_low_states[mask],
                                             l_actions=obn_actions[mask])

            obn_low_probs[mask] = o_obn_probs

        return obn_low_probs
    ...
    def _get_td_error(self,
                      ...
                      n_low_obses_list: list[torch.Tensor],
                      nx_low_target_obses_list: list[torch.Tensor],
                      low_state: torch.Tensor,
                      nx_low_target_states: torch.Tensor,
                      n_actions: torch.Tensor,
                      n_rewards: torch.Tensor,
                      n_dones: torch.Tensor,
                      n_mu_probs: torch.Tensor) -> torch.Tensor:
        """
        Args:
            next_n_vs_over_options: [batch, n, num_options]

            n_last_masks (torch.bool): [batch, n]
            n_padding_masks (torch.bool): [batch, n]
            n_option_indexes (torch.int64): [batch, n]
            n_low_obses_list: list([batch, n, *low_obs_shapes_i], ...)
            nx_low_target_obses_list: list([batch, n + 1, *low_obs_shapes_i], ...)
            low_state: [batch, low_state_size]
            nx_low_target_states: [batch, n + 1, low_state_size]
        ...
        td_error = torch.zeros((option_index.shape[0], 1), device=self.device)
        ...
        for i, option in enumerate(self.option_list):
            ...
            o_n_padding_masks = torch.logical_or(n_padding_masks[mask],
                                                 n_option_indexes[mask] != i)

            o_nx_low_obses_list = [torch.concat([n_low_obses[mask], nx_low_target_obses[mask, -1:]], dim=1)
                                   for n_low_obses, nx_low_target_obses
                                   in zip(n_low_obses_list, nx_low_target_obses_list)]

            o_td_error = option._get_td_error(
                next_n_vs_over_options=next_n_vs_over_options[mask],

                n_last_masks=n_last_masks[mask],
                n_padding_masks=o_n_padding_masks,
                nx_obses_list=o_nx_low_obses_list,
                nx_target_obses_list=[nx_low_target_obses[mask] for nx_low_target_obses in nx_low_target_obses_list],
                state=low_state[mask],
                nx_target_states=nx_low_target_states[mask],
                n_actions=n_actions[mask],
                n_rewards=n_rewards[mask],
                n_dones=n_dones[mask],
                n_mu_probs=n_mu_probs[mask])

            td_error[mask] = o_td_error

        return td_error

    def get_low_obnx_data(self,
        ...
        Returns:
            obnx_indexes: [batch, ob + n + 1]
            obnx_padding_masks: [batch, ob + n + 1]
            obn_option_indexes (torch.int64): [batch, ob + n + 1]
            obnx_pre_actions: [batch, ob + n + 1, action_size]
        """

        bnx_indexes = torch.concat([bn_indexes[:, self.option_burn_in_from:],
                                    bn_indexes[:, -1:] + (bn_indexes[:, -1:] != -1)], dim=1)
        bnx_padding_masks = torch.concat([bn_padding_masks[:, self.option_burn_in_from:],
                                          bn_padding_masks[:, -1:]], dim=1)
        bnx_option_indexes = torch.concat([bn_option_indexes[:, self.option_burn_in_from:],
                                           bn_indexes[:, -1:]], dim=1)
        bnx_pre_actions = gen_n_pre_actions(bn_actions[:, self.option_burn_in_from:], keep_last_action=True)

        return bnx_indexes, bnx_padding_masks, bnx_option_indexes, bnx_pre_actions

    def _train_q(self,
                 bn_indexes: torch.Tensor,
                 bn_last_masks: torch.Tensor,
                 bn_padding_masks: torch.Tensor,
        ...
        Args:
            bn_indexes (torch.int32): [batch, b + n],
            bn_last_masks (torch.bool): [batch, b + n],
            bn_padding_masks (torch.bool): [batch, b + n],
            bnx_target_states: [batch, b + n + 1]
            bn_option_indexes (torch.int64): [batch, b + n],
            obnx_low_obses_list: list([batch, ob + n + 1, *low_obs_shapes_i], ...)
        ...
        next_n_states = bnx_target_states[:, self.burn_in_step + 1:]
        ...
        (obnx_indexes,
         obnx_padding_masks,
         obnx_option_indexes,
         obnx_pre_actions) = self.get_low_obnx_data(
        ...
        obnx_low_states, _ = self.get_obnx_low_states(
            obnx_indexes=obnx_indexes,
            obnx_padding_masks=obnx_padding_masks,
            obnx_low_obses_list=obnx_low_obses_list,
            obnx_option_indexes=obnx_option_indexes,
            obnx_pre_actions=obnx_pre_actions,
            obnx_pre_low_seq_hidden_states=obnx_pre_low_seq_hidden_states,
            is_target=False
        )
        with torch.no_grad():
            obnx_low_target_states, _ = self.get_obnx_low_states(
                obnx_indexes=obnx_indexes,
                obnx_padding_masks=obnx_padding_masks,
                obnx_low_obses_list=obnx_low_target_obses_list,
                obnx_option_indexes=obnx_option_indexes,
                obnx_pre_actions=obnx_pre_actions,
                obnx_pre_low_seq_hidden_states=obnx_pre_low_seq_hidden_states,
                is_target=True
        ...
        for i, option in enumerate(self.option_list):
            ...
            o_n_padding_masks = torch.logical_or(bn_padding_masks[mask, self.burn_in_step:],
                                                 bn_option_indexes[mask, self.burn_in_step:] != i)

            o_d_y, o_c_y = option.compute_rep_q_grads(
                next_n_vs_over_options=next_n_vs_over_options[mask],

                n_indexes=bn_indexes[mask, self.burn_in_step:],
                n_last_masks=bn_last_masks[mask, self.burn_in_step:],
                n_padding_masks=o_n_padding_masks,
                nx_obses_list=[obnx_low_obses[mask]
                               for obnx_low_obses in obnx_low_obses_list],
                nx_target_obses_list=[obnx_low_target_obses[mask, self.option_burn_in_step:]
                                      for obnx_low_target_obses in obnx_low_target_obses_list],
                nx_states=obnx_low_states[mask, self.option_burn_in_step:],
                nx_target_states=obnx_low_target_states[mask, self.option_burn_in_step:],
                n_actions=bn_actions[mask, self.burn_in_step:],
                n_pre_actions=bn_actions[mask, self.burn_in_step - 1:-1],
                n_rewards=bn_rewards[mask, self.burn_in_step:],
                n_dones=bn_dones[mask, self.burn_in_step:],
                n_mu_probs=bn_mu_probs[mask, self.burn_in_step:],
                n_pre_seq_hidden_states=obnx_pre_low_seq_hidden_states[mask, self.option_burn_in_step:],
    ...
    def _train_v(self,
        ...
        state = nx_states[:, 0]
        option_index = n_option_indexes[:, 0]
        ...
        loss_v = torch.stack(loss_v_list).sum()
        ...
        if self.siamese is not None:
            loss_siamese, loss_siamese_q = self._train_siamese_representation_learning(
                ...
                n_obses_list=[nx_obses[:, :-1] for nx_obses in nx_obses_list],
                n_pre_actions=n_pre_actions,
                n_pre_seq_hidden_states=n_pre_seq_hidden_states)
    ...
    def _train_siamese_representation_learning(self,
                                               ...
                                               n_pre_actions: torch.Tensor,
                                               n_pre_seq_hidden_states: torch.Tensor) -> tuple[torch.Tensor,
                                                                                               torch.Tensor | None]:
        """
        Args:
            grads_rep_main list(torch.Tensor)
        ...
        if self.siamese_use_q:
            ...
            pre_actions_at_n = n_pre_actions[:, 0:1, ...]
            pre_seq_hidden_states_at_n = n_pre_seq_hidden_states[:, 0:1, ...]

            if self.seq_encoder in (None, SEQ_ENCODER.RNN):
                padding_masks_at_n = n_padding_masks[:, 0:1]
    ...
    def _train_terminations(self,
                            ...
                            low_state: list[torch.Tensor],
                            done: torch.Tensor,
                            priority_is: torch.Tensor | None) -> list[torch.Tensor]:
        """
        Args:
            y: [batch, 1]
        ...
        with torch.no_grad():
            ...
            target_v_over_options, _ = stacked_v_over_options.min(dim=0)  # [batch, num_options]
    ...
    def _train_rnd(self,
                   n_padding_masks: torch.Tensor,
                   n_option_indexes: torch.Tensor,
                   n_states: torch.Tensor) -> torch.Tensor:
    ...
    def _train(self,
        ...
        Returns:
            ...
            obnx_low_states: [batch, ob + n + 1, low_state_size]
            next_obnx_low_seq_hidden_states: [batch, ob + n + 1, *low_seq_hidden_state_shape]
            obnx_low_target_obses_list: list([batch, ob + n + 1, *low_obs_shapes_i], ...)
            obnx_low_target_states: [batch, ob + n + 1, low_state_size]
            next_n_vs_over_options: [batch, n, num_options]
        """
        ...
        (loss_v_list,
         ...
         loss_siamese_q) = self._train_v(y=y,
                                         ...
                                         n_indexes=bn_indexes[:, self.burn_in_step:],
                                         ...
                                         nx_states=bnx_states[:, self.burn_in_step:],
                                         n_option_indexes=bn_option_indexes[:, self.burn_in_step:],
                                         n_pre_actions=bn_actions[:, self.burn_in_step - 1:-1],
                                         n_pre_seq_hidden_states=bnx_pre_seq_hidden_states[:, self.burn_in_step:-1],
                                         priority_is=priority_is)
        ...
        with torch.no_grad():
            ...
            obnx_low_states, next_obnx_low_seq_hidden_states = self.get_obnx_low_states(
                obnx_indexes=obnx_indexes,
                obnx_padding_masks=obnx_padding_masks,
                obnx_low_obses_list=obnx_low_obses_list,
                obnx_option_indexes=obnx_option_indexes,
        ...
        self._train_terminations(y=y,
                                 ...
                                 low_obs_list=[obnx_low_obses[:, self.option_burn_in_step]
                                               for obnx_low_obses in obnx_low_obses_list],
                                 low_state=obnx_low_states[:, self.option_burn_in_step],
        ...
        for i, option in enumerate(self.option_list):
            ...
            option.train_policy_alpha(n_padding_masks=bn_padding_masks[mask, self.burn_in_step:],
                                      n_obses_list=[obnx_low_obses[mask, self.option_burn_in_step:-1, ...]
                                                    for obnx_low_obses in obnx_low_obses_list],
                                      nx_states=obnx_low_states[mask, self.option_burn_in_step:],
                                      n_actions=bn_actions[mask, self.burn_in_step:],
                                      n_mu_probs=bn_mu_probs[mask, self.burn_in_step:])
        ...
        return (bnx_states,
                next_bnx_seq_hidden_states,
                obnx_low_states,
                next_obnx_low_seq_hidden_states,
    ...
    def log_episode(self, force: bool = False, **episode_trans: np.ndarray) -> None:
        ...
        if self.summary_writer is not None and self.seq_encoder == SEQ_ENCODER.ATTN:
            with torch.no_grad():
                if self.use_dilation:
                    ...
                    torch_obs_list = [torch.from_numpy(o).to(self.device) for o in l_obses_list]
                    ...
                    *_, attn_weights_list = self.model_rep(ep_len,
                                                           l_indexes,
                                                           [_l_option_indexes] + torch_obs_list,
                                                           None,
                                                           None,
                                                           query_only_attend_to_rest_key=True)
        ...
        #     ep_pre_actions = gen_n_pre_actions(ep_actions_tensor)
    ...
    def put_episode(self,
        ...
        ep_last_masks = np.zeros_like(ep_indexes, dtype=bool)
        ep_last_masks[:, -1] = True  # The last step is next_step
        ep_last_masks[ep_indexes == -1] = True

        if self.use_replay_buffer:
            self._fill_replay_buffer(ep_indexes=ep_indexes,
                                     ep_last_masks=ep_last_masks,
    ...
    def _sample_from_replay_buffer(self) -> tuple[np.ndarray,
        ...
        Returns:
            ...
            (
                ...
                bn_option_indexes (torch.int8): [batch, b + n]
                bn_actions (torch): [batch, b + n, action_size]
                bn_rewards (torch): [batch, b + n]
                bn_dones (torch): [batch, b + n]
                bn_mu_probs (torch): [batch, b + n, action_size]
                bnx_pre_seq_hidden_states (torch): [batch, b + n + 1, *seq_hidden_state_shape],
                bnx_pre_low_seq_hidden_states (torch): [batch, b + n + 1, *low_seq_hidden_state_shape],
        ...
        batch:
            ...
            last_mask (bool): [batch, bn + 1]
            obs_i: [batch, bn + 1, *obs_shapes_i]
            option_index (torch.int8): [batch, bn + 1]
            option_changed_index (torch.int32): [batch, bn + 1]
        ...
        batch['padding_mask'] = torch.zeros_like(batch['last_mask'])
        ...
        batch['option_index'][invalid_mask] = -1
        batch['option_changed_index'][invalid_mask] = 0
        ...
        batch['pre_low_seq_hidden_state'][invalid_mask] = 0.

        """
        bnx_indexes (torch.int32): [batch, bn + 1]
        bnx_last_masks (bool): [batch, bn + 1]
        bnx_padding_masks (bool): [batch, bn + 1]
        bnx_obses_list: list([batch, bn + 1, *obs_shapes_i], ...)
        bnx_option_indexes (torch.int8): [batch, bn + 1]
        ...
        bnx_last_masks = batch['last_mask']
        ...
        bn_last_masks = bnx_last_masks[:, :-1]
        ...
        if self.use_dilation:
            ...
            for i in range(self.key_max_length, 0, -1):  # All keys are the first keys in episodes
                ...
                key_trans['padding_mask'][:, i - 1] = padding_mask
            ...
            key_trans['index'][key_trans['padding_mask']] = -1
            key_batch = (
                torch.from_numpy(key_trans['index']).to(self.device),
                torch.from_numpy(key_trans['padding_mask']).to(self.device),
                [torch.from_numpy(key_trans[f'obs_{name}']).to(self.device) for name in self.obs_names],
                torch.from_numpy(key_trans['option_index']).to(self.device),
                torch.from_numpy(key_trans['pre_seq_hidden_state']).to(self.device)
            )
    ...
    def train(self) -> int:
        ...
        if self.use_replay_buffer:
            ...
            batch, key_batch = self.batch_buffer.get_batch()
            if batch is None:
                self._profiler('train a step').ignore()
                return step
        ...
        priority_is: [batch, 1]
        """
        assert bn_indexes.dtype == torch.int32
        assert bn_option_indexes.dtype == torch.int8

        bn_option_indexes = bn_option_indexes.to(torch.int64)
        ...
        if self.use_replay_buffer:
            ...
            obn_low_states = obnx_low_states[:, :-1, ...]  # [batch, ob + n, *low_state_shape]
            next_bn_seq_hidden_states = next_bnx_seq_hidden_states[:, :-1, ...]  # [batch, b + n, *seq_hidden_state_shape]
            next_obn_low_seq_hidden_states = next_obnx_low_seq_hidden_states[:, :-1, ...]  # [batch, ob + n, *low_seq_hidden_state_shape]

            bn_low_obses_list = self.get_l_low_obses_list(l_obses_list=bn_obses_list,
                                                          l_states=bnx_states[:, :-1])

            if self.use_n_step_is or (self.d_action_sizes and not self.discrete_dqn_like):
                with self._profiler('get_obn_probs', repeat=10):
                    obn_pi_probs_tensor = self.get_obn_probs(
                        ...
                        obn_actions=bn_actions[:, self.option_burn_in_from:])
            ...
            if self.use_priority:
                with self._profiler('get_td_error', repeat=10):
                    td_error = self._get_td_error(
                        ...
                        n_last_masks=bn_last_masks[:, self.burn_in_step:],
                        n_padding_masks=bn_padding_masks[:, self.burn_in_step:],
                        n_option_indexes=bn_option_indexes[:, self.burn_in_step:],
                        n_low_obses_list=[bn_low_obses[:, self.burn_in_step:] for bn_low_obses in bn_low_obses_list],
                        nx_low_target_obses_list=[obnx_low_target_obses[:, self.option_burn_in_step:] for obnx_low_target_obses in obnx_low_target_obses_list],
                        low_state=obn_low_states[:, self.option_burn_in_step],
                        nx_low_target_states=obnx_low_target_states[:, self.option_burn_in_step:],
            ...
            bn_padding_masks = torch.logical_or(bn_padding_masks,
                                                bn_option_indexes != bn_option_indexes[:, self.burn_in_step:self.burn_in_step + 1])
