...
class AddTransitionBuffer:
    def __init__(self, add_trans):
        ...
        self.logger = logging.getLogger('ds.actor.add_transition_buffer')

        ts = [threading.Thread(target=self.run, daemon=True) for _ in range(5)]
        for t in ts:
            t.start()
...
class Actor(object):
    ...
    def __init__(self, root_dir, config_dir, args):
        ...
        # but the learner/replay stub will be generated by evolver
        self._evolver_stub = EvolverStubController(net_config['evolver_host'],
                                                   net_config['evolver_port'])

        self._sac_actor_lock = ReadWriteLock(5, 1, 1, logger=self.logger)
        ...
        try:
            ...
            (learner_host, learner_port,
             replay_host, replay_port) = evolver_register_response
            self.logger.info(f'Assigned to learner {learner_host}:{learner_port}')
            self.logger.info(f'Assigned to replay {replay_host}:{replay_port}')

            # The learner/replay stub is generated by evovler
            self._stub = StubController(learner_host, learner_port,
                                        replay_host, replay_port)

            # Learner or replay is offline, waiting for connection
            self.logger.warning('Waiting for learner or replay connection')
            while not self._stub.connected and self._evolver_stub.connected:
                time.sleep(C.RECONNECTION_TIME)
                continue
    ...
    def _run(self):
        ...
        while self._stub.connected and self._evolver_stub.connected:
            ...
            if self.base_config['update_policy_mode'] \
                ...
                self.logger.info('start _update_policy_variables')
                self._update_policy_variables()
                self.logger.info('_update_policy_variables')
...
class EvolverStubController:
    ...
    def __init__(self, evolver_host, evolver_port):
        ...
        t_evolver = threading.Thread(target=self._start_persistence, daemon=True)
        t_evolver.start()
    ...
    def register(self):
        ...
        if response.succeeded:
            return (response.learner_host, response.learner_port,
                    response.replay_host, response.replay_port)
