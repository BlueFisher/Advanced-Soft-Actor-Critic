...
class ModelRep(m.ModelBaseRNNRep):
    def _build_model(self, blur, brightness, ray_random, need_speed):
        ...
        assert self.obs_shapes[3] == (25, 7)  # ThirdPersonBoundingBoxSensor
...
class ModelOptionRep(m.ModelBaseRNNRep):
    def _build_model(self, blur, brightness, ray_random, need_speed):
        ...
        self.rnn = m.GRU(64 + 32 + self.obs_shapes[6][0] + self.c_action_size, 64, 1)
    ...
    def forward(self, obs_list, pre_action, rnn_state=None):
        ...
        state, hn = self.rnn(torch.cat([high_state, ray + bbox_cam + bbox_tp_cam, vec, pre_action], dim=-1),
                             rnn_state)
